    ...
    , m_loginDialog(nullptr)
{
   ...
   // setupTagManager();
    ...
    // 2. 监听设备列表的变化信号
    // 当 TagConfigDialog 增删设备后，主界面的下拉框需要同步更新
    connect(m_plcData, &PLCData::dataChanged, this, &MainWindow::refreshDeviceComboBox);
    ...
    QTimer::singleShot(100, this, [this]() {
        ...
        //qDebug() << "强制重置后登录状态:" << UserManager::instance().isLoggedIn();
...
{
    // 断开与 PLCData 的数据更新连接，避免对象销毁后仍被调用
    if (m_plcData) {
...
void MainWindow::setupUI()
{
    ...
    // 设置 mainSplitter 的初始分割比例
    // 假设主窗口宽度为 1600px
    // 左侧导航栏: 200px，主区域: 1200px，右侧标签管理器: 200px
    ui->mainSplitter->setSizes(QList<int>() << 200 << 1200 << 200);
    ...
    // 设置 dockWidget 的初始和最小宽度
    ui->dockTagManager->setMinimumWidth(150);
    ui->dockTagManager->setMaximumWidth(300);
...
void MainWindow::performOneTimeInitialization() {
  ...
  //  loadThemeSettings();
    ...
    // 4. 图表系统初始化
    // 关键：确保 ui->chartWidget 已经存在
    if (ui->chartWidget) {
        setupCharts();
    } else {
        qWarning() << "ui->chartWidget 为空，图表初始化失败！";
    }

    if (ui->historyChartWidget) {
        setuptrend();
    } else {
        qWarning() << "ui->historyChartWidget 为空，趋势图初始化失败！";
    }
...
// 打开配置对话框
void MainWindow::on_actionTagConfig_triggered()
{
    // 以模态方式打开配置对话框
    TagConfigDialog dialog(m_plcData, this);

    // 如果用户在对话框中点击了“确定”
    if (dialog.exec() == QDialog::Accepted) {
        // 保存配置到文件（假设 PLCData 有这个方法）
        m_plcData->saveToConfig("config.json");

        // 刷新主界面的设备下拉框
        refreshDeviceComboBox();

        qDebug() << "MainWindow: 标签配置已更新并保存";
    }
}
...
{
    // 防止在清空和添加过程中触发 currentIndexChanged 导致崩溃
    ui->comboBoxDevices->blockSignals(true);
    ui->comboBoxDevices->clear();

    auto devices = m_plcData->getAllDevices();
    for (const auto& device : devices) {
        // 将设备名称显示在下拉框，设备ID作为隐藏数据
        ui->comboBoxDevices->addItem(device.name, device.id);
    }

    ui->comboBoxDevices->blockSignals(false);

    // 如果有设备，默认选中第一个并加载对应视图
    if (devices.size() > 0) {
...
void MainWindow::on_comboBoxDevices_currentIndexChanged(int index)
{
    ...
    qDebug() << "MainWindow: 切换到设备 ->" << deviceId;

    // TODO: 通知 ChartManager 和 RealTimeTableModel 刷新
    // 例如：m_chartManager->setActiveDevice(deviceId);
    // 例如：m_tableModel->setActiveDevice(deviceId);
}