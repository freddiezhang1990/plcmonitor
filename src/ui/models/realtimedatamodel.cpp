...
QVariant RealTimeDataModel::data(const QModelIndex &index, int role) const
{
    ...
    if (tagIndex >= 0 && tagIndex < m_visibleTags.size()) {
        QString tagKey = m_visibleTags[tagIndex];
        if (record.tagValues.contains(tagKey)) {
            return record.tagValues[tagKey];
        }
    }
...
QVariant RealTimeDataModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    ...
    if (orientation == Qt::Horizontal) {
        QMutexLocker locker(&m_dataMutex);
        if (section == 0) return tr("时间戳");
        int tagIndex = section - 1;
        if (tagIndex >= 0 && tagIndex < m_visibleTags.size()) {
            return getDisplayName(m_visibleTags[tagIndex]);
        }
...
QString RealTimeDataModel::getDisplayName(const QString& tagKey) const
{
    int slash = tagKey.indexOf('/');
    if (slash == -1) return tagKey;
    // 可以调用外部函数获取设备名称，为了简单，先返回 "[设备ID] 标签名"
...
void RealTimeDataModel::setVisibleTags(const QStringList &tagKeys)
{
    QMutexLocker locker(&m_dataMutex);
    m_visibleTags = tagKeys;
    rebuildColumnIndex();
    locker.unlock();
...
void RealTimeDataModel::addTagColumn(const QString &tagKey)
{
    if (m_visibleTags.contains(tagKey))
        return;

    QMutexLocker locker(&m_dataMutex);

    m_visibleTags.append(tagKey);
    rebuildColumnIndex();
...
void RealTimeDataModel::removeTagColumn(const QString &tagKey)
{
    int index = m_visibleTags.indexOf(tagKey);
    if (index == -1) return;
